...
class AWCS:
    def __init__(self, dhtPins=[5,6], lcdAddr=[0x27, 0x26], buttonsPins=[7,8]):
        self.lcdSize = (2, 16)
        ...
        self.dht_sensorNo = dht.DHT11(Pin(dhtPins[1], Pin.IN, Pin.PULL_UP))

        self.sys_switch = Pin(buttonsPins[0], Pin.IN, Pin.PULL_UP)
    ...
    def display_data(self, datas):
        for i in range(2):
            line1 = f"Home Temp: {datas[i][0]}C"
            line2 = f"Home Humidity: {datas[i][1]}%"
